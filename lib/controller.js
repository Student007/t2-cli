  ...
  return new Promise(function(resolve, reject) {
    ...
    seeker.on('tessel', function displayResults(tessel) {
      ...
      if (foundTessels.length >= 1) {
        for (var i in foundTessels) {
          if (foundTessels[i].connections[0].ip === tessel.connections[0].ip && tessel.connections[0].connectionType === 'LAN') {
        ...
        controller.runHeuristics(opts, foundTessels)
          .then(function logSelected(tessel) {
            ...
            if (tessel) {
              ...
              controller.menu.prepareMenu(opts, foundTessels, rtm, resolve, reject,'Please select the default Tessel 2', function(name,resolve,reject){
                logs.info('Selected Tessel: '+name);
                logs.err('TODO: ... we need to persist the selected Tessel!');
                resolve();
              });
...
controller.menu = {
  ...
  prepareMenu: function(opts, tessels, rtm, resolve, reject, title, callback) {
  ...
  runSSH: function(id, opts, tessels, resolve, reject) {
    ...
    if (opts.menu) {
      ...
      var ch = require('child_process')
        .spawn('ssh', ['-i',
          opts.path,
          'root@' + tessels[id].connections[0].ip
        ], {
          stdio: 'inherit'
        });
