...
actions.setDefaultKey = function(opts) {
  return new Promise(function(resolve, reject) {
    if (opts.key && typeof opts.key === 'string') {
      ...
      reject('No valid path for private key');
    }
    try {
      if (fs.statSync(authKey).isFile() &&
        fs.statSync(authKey + '.pub').isFile()) {
        debug('Keyfiles found!');
        resolve();
      }
    } catch (e) {
      debug('Catched error: Keyfiles not found', e);
      reject('Could not find ' + authKey + ' and its public key (.pub)');
    }
  });
};
