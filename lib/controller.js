...
controller.update = function(opts) {
  return new Promise(function(resolve, reject) {
    // If the user requested a list of builds
    if (opts.list) {
      // Fetch those builds from the build server
      return updates.fetchBuildList()
        .then(function(builds) {
          logs.info('Latest builds:');
          // Print out details for each build
          builds.forEach(function(build) {
            logs.basic('\t' + build.version);
          });
          // Finish up
          return resolve();
        }, reject);
      ...
      var version = 'latest';
      // Unless a specific build was request
      if (opts.build) {
        // Then we'll fetch that build
        version = opts.build;
      }
      // Fetch that version
      updates.fetchBuild(version)
        // Once we have the build
        .then(function(build) {
          // Grab our Tessel
          return Tessel.get(opts)
            // Once we have the Tessel
            .then(function(tessel) {
              // Figure out what build is running on it
              return tessel.fetchCurrentBuildInfo()
                // Once we have the current build
                .then(function(current) {
                  // If the update is forced or this build was requested
                  if (opts.force || opts.build) {
                    // Just update it
                    return tessel.update(build.binaries)
                      // Log that the update completed
                      .then(logCompletion.bind(this, tessel.name, current.version, build.version), reject)
                      .then(function() {
                    ...
                    // Check if the current build is the same or newer
                    if (current.version >= build.version) {
                      var errMessage = tessel.name + ' is already on the latest firmware build. You can force an update with “tessel update --force”';
                      return controller.closeTesselConnections([tessel])
                        .then(reject.bind(this, errMessage), reject.bind(this, errMessage));
                    } else {
                      // If it is a newer version, let's update...
                      logs.info('New firmware version found...' + current.version);
                      logs.info('Updating ' + tessel.name + ' to latest build...');
                      return tessel.update(build.binaries)
                        // Log that the update completed
                        .then(logCompletion.bind(this, tessel.name, current.version, build.version), reject)
                  ...
                  // Close the Tessel connection
                  return controller.closeTesselConnections([tessel])
                    // Then reject the update whether or not the closing succeeded or failed
                    .then(reject.bind(err), reject.bind(err));
                });
            }, reject);
        }, reject);
    }
  ...
  function logCompletion(name, oldVersion, newVersion) {
    logs.info('Updated ' + name + ' from ' + oldVersion + ' to ' + newVersion);
  }
};
